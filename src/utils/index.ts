import { MAX_COLS, MAX_ROWS, NO_OF_BOMBS } from "../constance";
import { CellValue, CellState, Cell } from "../types";

const grabAllAdhacentCells = (
  cells: Cell[][],
  rowParam: number,
  colParam: number
): {
  ...
  for (let rowIndex = 0; rowIndex < MAX_ROWS; rowIndex++) {
    for (let colIndex = 0; colIndex < MAX_COLS; colIndex++) {
      ...
      const {
        ...
        bottomCell,
        bottomRightCell,
      } = grabAllAdhacentCells(cells, rowIndex, colIndex);
...
export const openMultipleCells = (
  ...
  const {
    ...
    bottomCell,
    bottomRightCell,
  } = grabAllAdhacentCells(cells, rowParam, colParam);
